  ...
  elseif self.wall then
    ...
    elseif jumpActivated then
      ...
      if self.wallReleaseTimer > self.wallReleaseTime then
        ...
        elseif self.wallGrabFreezeTimer == 0 then
            mcontroller.controlParameters({
                gravityEnabled = false
            })
            if self.wall == "right" and args.moves["right"] then
                mcontroller.controlApproachYVelocity(12, 500)
                tech.setParentState("walk")
            elseif self.wall == "left" and args.moves["left"] then
                mcontroller.controlApproachYVelocity(12, 500)
                tech.setParentState("walk")
            elseif (args.moves["left"] == false) and (args.moves["right"] == false) then
                if status.statusProperty("wallSlide") == true then
                    mcontroller.controlApproachYVelocity(-4, 500)
...
function grabWall(wall)
  ...
  --mcontroller.controlRotation(95.0)
  status.setStatusProperty("wallSlide", true)
end
